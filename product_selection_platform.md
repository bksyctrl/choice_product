...
商品列表需要按商品唯一 ID 去重展示。去重规则为：同一个 `product_id` / `商品ID` 在数据库中存在多个 `date_record` 快照时，前端列表只展示一条记录。

- 如果用户没有选择采集日期范围，则展示该商品全库最新 `date_record` 的数据。
- 如果用户选择了采集日期范围（例如 `date_start=2026-05-18`、`date_end=2026-05-21`），则在该范围内取该商品最新 `date_record` 的数据。
- 如果同一商品在同一最新采集日期下仍有多条记录，则取数据库自增 `id` 最大的一条。
- 统计概览接口与商品列表接口使用同一套去重口径，避免列表数量与统计数量不一致。

示例：`product_id=123` 同时存在 `2026-05-21`、`2026-05-22`、`2026-05-23`、`2026-05-24` 数据。用户选择采集日期 `2026-05-21` 到 `2026-05-23` 时，列表展示 `2026-05-23` 的数据；用户不选择采集日期范围时，列表展示 `2026-05-24` 的数据。
